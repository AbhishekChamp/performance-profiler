    ...
    <div className="w-full">
      ...
      >
        ...
        >
          ...
          <div className="absolute -inset-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-2xl opacity-20 group-hover:opacity-40 blur-xl transition-opacity duration-500" />
          ...
          >
            ...
            <div className="relative mb-6">
              <div 
                className={`
                  ...
                  ${isDragging 
                    ? 'bg-gradient-to-br from-cyan-500 to-blue-600 scale-110 rotate-6 shadow-2xl shadow-cyan-500/50' 
                    : 'bg-gradient-to-br from-gray-700 to-gray-800 group-hover:scale-105 group-hover:-translate-y-2 shadow-xl shadow-black/30'
                  }
                `}
                style={{
              ...
              >
                {isFolderMode ? (
                  <FolderUp className={`w-12 h-12 ${isDragging ? 'text-white' : 'text-gray-400 group-hover:text-white'} transition-colors`} />
                ) : (
                  <Upload className={`w-12 h-12 ${isDragging ? 'text-white' : 'text-gray-400 group-hover:text-white'} transition-colors`} />
                )}
                
                {/* Floating Particles */}
                <div className="absolute -top-2 -right-2 w-4 h-4 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '0s', animationDuration: '2s' }} />
                <div className="absolute -bottom-1 -left-3 w-3 h-3 bg-purple-400 rounded-full animate-bounce" style={{ animationDelay: '0.5s', animationDuration: '2.5s' }} />
                <div className="absolute top-0 -left-4 w-2 h-2 bg-pink-400 rounded-full animate-bounce" style={{ animationDelay: '1s', animationDuration: '3s' }} />
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <button
                onClick={() => isFolderMode ? folderInputRef.current?.click() : fileInputRef.current?.click()}
                className="
      ...
      {files?.length > 0 && (
        <div className="mt-8">
          <div className="bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-gray-700/50 overflow-hidden shadow-2xl shadow-black/30">
            {/* Header */}
            <div className="px-6 py-4 border-b border-gray-700/50 flex items-center justify-between bg-gradient-to-r from-gray-800/50 to-transparent">
              ...
              {files?.map((file, index) => (
                ...
                >
                  ...
                  {file.path && file.path !== file.name && (
                    <span className="text-xs text-gray-600 truncate max-w-[200px] hidden sm:block">
                      {file.path}
                    </span>
                  )}
            ...
            {/* Analyze Button */}
            <div className="px-6 py-4 border-t border-gray-700/50 bg-gradient-to-r from-gray-800/30 to-transparent">
              <button
                onClick={onAnalyze}
                className="
                  w-full py-4 
