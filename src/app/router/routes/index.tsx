    ...
    >
      ...
      >
        ...
        <div className="relative">
          <div className={`w-14 h-14 rounded-xl ${color.replace('bg-gradient-to-br', '').replace('from-', 'bg-').split(' ')[0]} bg-opacity-20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
            <Icon className={`w-7 h-7 ${color.includes('blue') ? 'text-blue-400' : color.includes('purple') ? 'text-purple-400' : color.includes('pink') ? 'text-pink-400' : color.includes('cyan') ? 'text-cyan-400' : 'text-emerald-400'}`} />
          </div>
  ...
  return (
    <div className="relative text-center mb-12">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] pointer-events-none">
        ...
        <div className="absolute inset-8 border border-purple-500/20 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }} />
        <div className="absolute inset-16 border border-pink-500/20 rounded-full animate-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute inset-24 border border-cyan-500/20 rounded-full animate-pulse" style={{ animationDelay: '1.5s' }} />
      </div>
      ...
      <div className="relative inline-block mb-8">
        ...
        >
          {/* Main Logo */}
          <div 
            className="absolute inset-0 rounded-3xl bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 flex items-center justify-center shadow-2xl shadow-purple-500/30"
          ...
          <div 
            className="absolute inset-y-0 left-0 w-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-l-3xl"
            style={{ transform: 'rotateY(-90deg) translateZ(64px)' }}
          />
          <div 
            className="absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-purple-600 to-pink-600 rounded-r-3xl"
            style={{ transform: 'rotateY(90deg) translateZ(64px)' }}
          />
          <div 
            className="absolute inset-x-0 top-0 h-8 bg-gradient-to-b from-blue-400 to-purple-400 rounded-t-3xl"
            style={{ transform: 'rotateX(90deg) translateZ(64px)' }}
          />
          <div 
            className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-purple-600 to-pink-600 rounded-b-3xl"
            style={{ transform: 'rotateX(-90deg) translateZ(64px)' }}
          />
      ...
      <p className="text-xl text-gray-400 max-w-2xl mx-auto mb-8">
        ...
        <span className="text-cyan-400 font-semibold">16 specialized analyzers</span>. 
        Detect bottlenecks, optimize bundles, and ship faster.
      </p>
...
function AnalysisProgress({ progress }: { progress: { message: string; progress: number } | null }) {
  return (
    <div className="flex flex-col items-center justify-center h-full relative">
      ...
      {progress && (
        <>
          ...
          {/* 3D Progress Bar */}
          <div className="w-80 h-4 bg-gray-800 rounded-full overflow-hidden relative" style={{ perspective: '500px' }}>
            <div 
...
function IndexComponent() {
  ...
  const handleAnalyze = async () => {
    ...
    toast.loading('Starting analysis...', { id: 'analysis', duration: Infinity });
      ...
      <div className="animate-in fade-in duration-500">
        {(() => {
          switch (activeSection) {
            ...
            case 'images':
              return currentReport.images ? <ImagesSection images={currentReport.images} /> : <NoData section="Images" />;
            case 'fonts':
              return currentReport.fonts ? <FontsSection fonts={currentReport.fonts} /> : <NoData section="Fonts" />;
            case 'assets':
            ...
            case 'webvitals':
              return currentReport.webVitals ? <WebVitalsSection webVitals={currentReport.webVitals} /> : <NoData section="Web Vitals" />;
            case 'network':
              return currentReport.network ? <NetworkSection network={currentReport.network} /> : <NoData section="Network" />;
            case 'accessibility':
              return currentReport.accessibility ? <AccessibilitySection accessibility={currentReport.accessibility} /> : <NoData section="Accessibility" />;
            case 'seo':
              return currentReport.seo ? <SEOSection seo={currentReport.seo} /> : <NoData section="SEO" />;
            case 'typescript':
              return currentReport.typescript ? <TypeScriptSection typescript={currentReport.typescript} /> : <NoData section="TypeScript" />;
            case 'security':
              return currentReport.security ? <SecuritySection security={currentReport.security} /> : <NoData section="Security" />;
            case 'thirdparty':
              return currentReport.thirdParty ? <ThirdPartySection thirdParty={currentReport.thirdParty} /> : <NoData section="Third-Party" />;
            case 'memory':
              return currentReport.memory ? <MemorySection memory={currentReport.memory} /> : <NoData section="Memory" />;
            case 'imports':
              return currentReport.imports ? <ImportsSection imports={currentReport.imports} /> : <NoData section="Imports" />;
            case 'timeline':
